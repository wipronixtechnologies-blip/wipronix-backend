  ...
  try {
    ...
    if (req.file) {
      ...
      if (existingStudent.profilePicture) {
        try {
          await imageService.deleteImage(existingStudent.profilePicture);
        } catch (deleteError) {
      ...
      try {
        const uploadResult = await imageService.uploadImage(req.file, 'profile-pictures');
        if (uploadResult.success) {
          filteredUpdateData.profilePicture = uploadResult.data.filePath;
        } else {
          return res.status(400).json({
            success: false,
    ...
    console.error('Update student with profile picture error:', error);
...
module.exports = updateStudentWithProfilePicture;
