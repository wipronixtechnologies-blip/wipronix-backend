const PassingYear = require('../../models/PassingYear.model');

const updatePassingYear = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, isActive } = req.body;

    const option = await PassingYear.findById(id);
    if (!option) {
      return res.status(404).json({
        success: false,
        message: 'Passing Year option not found'
      });
    }

    if (name && name.trim() && name.trim() !== option.name) {
      const existingOption = await PassingYear.findOne({
        _id: { $ne: id },
        name: { $regex: new RegExp(`^${name.trim()}$`, 'i') }
      });
      if (existingOption) {
        return res.status(400).json({
          success: false,
          message: 'Another Passing Year option with this name already exists'
        });
      }
      option.name = name.trim();
    }

    if (isActive !== undefined) {
      option.isActive = isActive;
    }

    await option.save();

    res.status(200).json({
      success: true,
      message: 'Passing Year option updated successfully',
      data: option
    });
  } catch (error) {
    console.error('Error updating Passing Year option:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to update Passing Year option',
      error: error.message
    });
  }
};

module.exports = updatePassingYear;
